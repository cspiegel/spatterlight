...
int isfullscreen(struct winctx ctx)
{
    return ((ctx.x0 == 0 && ctx.y0 == 0 && ctx.x0 == gscreenw && ctx.y0 == gscreenh)
            || (ctx.l == 1 && ctx.r == 0x7FFF && ctx.t == 1 && ctx.b == 0x7FFF));
}
            ...
            if (wins[i].win)
            {
                ...
                for (int j = 1; j < nwins; j++) {
                    if (wins[j].win && wins[j].win->type == wintype_Graphics && j != second_image_row && wins[j].wasmoved && isfutureboy) {
                        ...
                        if (overlap(wins[j], wins[i]) || overlap(wins[j], wins[curwin])) {
...
static void hugo_mapcurwin()
{
        ...
        (!mono && inwindow && wins[curwin].l > 5))  // We are in a right-aligned aux textbuffer window
    {
        ...
        if (!mono && inwindow && wins[curwin].l > 5) {
            if (wins[mainwin].y0 > wins[curwin].y1)
